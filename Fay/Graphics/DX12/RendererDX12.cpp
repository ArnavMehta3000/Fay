...
extern "C"
{
    // Run the app on the discrete GPU on NVIDIA Optimus laptops by default
...
namespace fay
{
    ...
    void RendererDX12::ReportLiveObjects()
    {
        ...
        {
            Log::Info("Reporting DXGI live objects");
        ...
        if (m_device)
        {
            nvrhi::RefCountPtr<ID3D12DebugDevice> d3dDebugDevice;
            if (SUCCEEDED(m_device->QueryInterface(IID_PPV_ARGS(&d3dDebugDevice))))
            {
                Log::Info("Valid device available, reporting DX12 live objects");
    ...
    bool RendererDX12::CreateDevice()
    {
        ...
        if (m_initInfo.EnableDebugRuntime)
        {
            ...
            {
                Log::Warn("Unable to enable DX12 debug layer");
            }
        }
        ...
        if (m_initInfo.EnableGPUValidation)
        {
            ...
            {
                Log::Warn("Unable to enable DX12 GPU validation");
            }
        }
        ...
        if (FAILED(m_dxgiFactory->EnumAdapters(adapterIndex, &m_dxgiAdapter)))
        {
            ...
            {
                Log::Error("Specified DXGI adapter {} does not exist", adapterIndex);
            }
    ...
    bool RendererDX12::CreateSwapChain()
    {
        ...
        {
            Log::Error("Failed to create render targets!");
            return false;
        }
    ...
    bool RendererDX12::CreateRenderTargets()
    {
        ...
        {
            HR_RETURN(m_swapChain->GetBuffer(i, IID_PPV_ARGS(&m_swapChainBuffers[i])));
